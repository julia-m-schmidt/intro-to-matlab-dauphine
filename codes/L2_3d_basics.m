...
%  The smallest possible example, small enough to check by hand:
xs = [1 2 3];               % 3 values, they run ACROSS the columns
ys = [10 20];               % 2 values, they run DOWN the rows
[Xs, Ys] = meshgrid(xs, ys);
Zs = Xs.*Ys                 % no semicolon - look at all three

% Xs =  1   2   3      Ys = 10  10  10      Zs = 10  20  30
%       1   2   3           20  20  20           20  40  60
%
% Read the SAME POSITION in all three. Position (2,3): Xs says 3, Ys says 20,
% Zs says 60 - that is the pair (3,20) and its answer. Every entry works this
% way. The grids are only meaningful read together, position by position.
%
% Count them: 3 x-values * 2 y-values = 6 pairs. 
